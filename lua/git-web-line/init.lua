...
function M.git_remote_host()
  ...
  -- e.g. github.com
  local remote_host = vim.fn.system("git config --get remote.origin.url | sed -n 's/.*@//;s/:.*//p' | tr -d '\n'")
  return remote_host
end
...
function M.git_remote_username()
  ...
  -- e.g. torvalds
  local remote_repo_username = vim.fn.system("git config --get remote.origin.url | sed -n 's/.*://;s/\\/.*//p' | tr -d '\n'")
...
function M.activate()
  ...
  local url = "https://" .. remote_host .. "/" .. remote_repo_username .. "/" .. remote_repo_name .. "/blob/" .. branch_name .. "/" .. file_path .. "#L" .. current_line
